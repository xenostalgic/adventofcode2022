    ...
    for t in range(1,time+1):
        ...
        for sidx in tqdm(valid_next):
            ...
            for pidx_a, pidx_b in itertools.product(range(p), range(p)):
                ...
                best_prev_idx = (t-1,sidx,pidx_a,pidx_b)
                best_idx_score = lattice[best_prev_idx]
                best_action_a = "wait at"
                best_action_b = "wait at"

                # cases:
                # a traveled 1 step, b traveled 1 step (from current sidx, max over pairs of positions)
                ppos_a = pidx_edges[pidx_a]
                ppos_b = pidx_edges[pidx_b]
                for prev_pidx_a, prev_pidx_b in itertools.product(ppos_a, ppos_b):
                    prev_travel_idx = (t-1,sidx,prev_pidx_a,prev_pidx_b)
                    prev_score = lattice[prev_travel_idx]
                    potential_score = prev_score + sflows[sidx]
                    if prev_score > -1 and potential_score >= best_idx_score:
                        best_idx_score = potential_score
                        best_prev_idx = prev_travel_idx
                        best_action_a = "move to"
                        best_action_b = "move to"
                
                # if a's valve is open, check: a turned a valve, b traveled 1 step (from prev_sidx given by valve)
                if vidx_a_present:
                    prev_sidx = sidx - (2**vidx_a)
                    for prev_pidx_b in ppos_b:
                        prev_score = lattice[t-1,prev_sidx,pidx_a,prev_pidx_b]
                        if prev_score == -1:
                            continue
                        prev_flow = sflows[prev_sidx] if prev_sidx in sflows else 0
                        potential_score = prev_score + prev_flow
                        if potential_score >= best_idx_score:
                            best_idx_score = potential_score
                            best_prev_idx = (t-1,prev_sidx,pidx_a,prev_pidx_b)
                            best_action_a = "turn valve at"
                            best_action_b = "move to"

                # if b's valve is open, check: b turned a valve, a traveled 1 step (from prev_sidx given by valve)
                if vidx_b_present:
                    prev_sidx = sidx - (2**vidx_b)
                    for prev_pidx_a in ppos_a:
                        prev_score = lattice[t-1,prev_sidx,prev_pidx_a,pidx_b]
                        if prev_score == -1:
                            continue
                        prev_flow = sflows[prev_sidx] if prev_sidx in sflows else 0
                        potential_score = prev_score + prev_flow
                        if potential_score >= best_idx_score:
                            best_idx_score = potential_score
                            best_prev_idx = (t-1,prev_sidx,prev_pidx_a,pidx_b)
                            best_action_a = "move to"
                            best_action_b = "turn valve at"
                
                # if both valves open, check: a turned a valve, b turned a valve (from prev_sidx given by valve pair)
                if vidx_a_present and vidx_b_present and vidx_a != vidx_b:
                    prev_sidx = sidx - (2**vidx_a) - (2**vidx_b)
                    prev_score = lattice[t-1,prev_sidx,pidx_a,pidx_b]
                    if prev_score == -1:
                        continue
                    prev_flow = sflows[prev_sidx] if prev_sidx in sflows else 0
                    potential_score = prev_score + prev_flow
                    if potential_score >= best_idx_score:
                        best_idx_score = potential_score
                        best_prev_idx = (t-1,prev_sidx,pidx_a,pidx_b)
                        best_action_a = "turn valve at"
                        best_action_b = "turn valve at"

                lattice[cur_idx] = best_idx_score
                #paths[cur_idx] = paths[best_prev_idx] + [(best_action_a + " " + pos_nodes[pidx_a], best_action_b + " " + pos_nodes[pidx_b])]
